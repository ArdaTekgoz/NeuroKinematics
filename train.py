"""
NeuroKinematics — Training Script (Checkpoint: A-4.3.2)
======================================================

Goal
----
Train a lightweight inverse-kinematics regressor that maps:
  input  X = [x, y, z, qx, qy, qz, qw]  (end-effector pose, meters + unit quaternion)
  target Y = [t1, t2, t3, t4, t5, t6]  (joint angles, radians)

This file is intentionally a **single, self-contained train.py**:
- The repo does not currently have a stable `core/` python package to import from.
- A single file makes this checkpoint easy to reproduce and review on GitHub.

What we keep from A-4.3.2 (working behavior)
--------------------------------------------
- Model architecture (MLP) and optimizer (Adam, lr=1e-3)
- Primary loss: joint-space MSE (theta_pred vs theta_gt)
- FK-based auxiliary loss: hard-sample weighted FK position error
- Training prints per-epoch summary similar to A-4.3.2

What we bring back from A-4.2 (structure & clarity) — without changing objectives
--------------------------------------------------------------------------------
- Explicit sections (Dataset / Model / Losses / Training Loop / Validation)
- Deterministic split + seed control
- Numerically safe loss weighting (eps, clamping, detach rationale)
- Robust dataset path handling (local repo default; no Colab-only assumptions)

Important note on "Forward Kinematics"
--------------------------------------
The FK function below is the same *placeholder* formulation used in A-4.3.2/A-4.2
(sum of trig terms). It is differentiable and serves to enforce consistency in
the synthetic pipeline.

If you later replace this with the robot's real DH-based FK, do it in ONE place:
`forward_kinematics_position(theta)`.
"""
