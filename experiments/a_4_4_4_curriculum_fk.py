...
This experiment extends A-4.3.3 (hybrid MSE + FK loss) by implementing
curriculum learning for the FK loss weight. Instead of a constant lambda_fk,
we gradually increase the FK weight from 0 to lambda_max over the course
of training.

Curriculum Learning Rationale:
- Early epochs: Model focuses primarily on MSE (joint angle prediction)
- Later epochs: FK constraint gradually increases, enforcing geometric consistency
- This progressive curriculum helps the model learn joint-space patterns first,
  then refine with FK constraints, potentially improving convergence and stability.

Schedule:
  lambda_fk(epoch) = lambda_max * (epoch / (epochs - 1))
  - Epoch 0: lambda_fk = 0.0 (pure MSE)
  - Epoch (epochs-1): lambda_fk = lambda_max (full FK weight)
"""
...
def train_one_epoch(
    ...
    """
    Train one epoch with curriculum FK weighting.
...
def validate_and_dump_fk_errors(
    ...
    """
    A-4.3.3 / A-4.4.4 validation analysis:
    ...
    print(
        "FK Error Stats (validation) — A-4.4.4:"
        f" mean={mean_err:.6f},"
        f" std={std_err:.6f},"
        f" p95={p95:.6f},"
        f" p99={p99:.6f},"
...
def save_summary_and_config(
    ...
    with open(summary_path, "w") as f:
        f.write("A-4.4.4 Curriculum FK Weighting - Validation Summary\n")
        f.write("=" * 60 + "\n\n")
    ...
    config_dict = {
        "experiment": "A-4.4.4",
        "description": "Curriculum FK Weighting",
        "args": vars(args),
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Train IKNet with Curriculum FK Weighting (A-4.4.4)")
    ...
    # A-4.4.4 validation output directory
    p.add_argument(
        "--dump_dir",
        type=str,
        default="analysis/a_4_4_4",
        help="Output directory for A-4.4.4 validation FK error dumps, summary.txt, and config.json.",
    )
...
def main() -> None:
    ...
    # Training loop with curriculum FK weighting
    print(f"\nA-4.4.4: Curriculum FK Weighting")
    ...
    for epoch in range(args.epochs):
        ...
        # Epoch logging: includes current lambda_fk for curriculum tracking
        print(
            f"[Epoch {epoch + 1:03d}] "
    ...
    # Validation (kept consistent with A-4.3.3)
    val_mse = validate_mse(model=model, loader=val_loader, mse_criterion=mse_criterion, device=device)
    print(f"\nValidation MSE: {val_mse:.6f}")

    # A-4.4.4 validation-time FK error dump and summary/config saving (always performed)
