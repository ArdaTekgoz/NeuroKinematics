# -*- coding: utf-8 -*-
"""
A-4.4.1 baseline: pure joint-space MSE (no FK supervision).

This script is intentionally identical to A-4.3.3 (`train_43.py`) in
data/model/optimizer/scheduler/seeding/splitting/logging style, except
for ONE change: training uses only joint MSE. FK is evaluated *after*
training for analysis only and never affects gradients.
"""
...
# ============================================================
# Forward Kinematics (Position) — analysis-only
# ============================================================
...
def forward_kinematics_position(theta: torch.Tensor) -> torch.Tensor:
    ...
    This is used *only* for validation-time analysis in this baseline.
    """
...
# ============================================================
# Training (MSE-only baseline — no FK supervision)
# ============================================================


def train_one_epoch_mse(
    model: nn.Module,
    loader: DataLoader,
    ...
    """
    Single-epoch training using only joint-space MSE.
    """
    model.train()
    total_mse = 0.0
    n_batches = 0
    ...
    for x, y in tqdm(loader, desc="Train", leave=False):
        ...
        # Only joint-space MSE contributes to gradients.
        mse_loss.backward()
        optimizer.step()

        total_mse += float(mse_loss.detach().cpu())
        n_batches += 1
...
def evaluate_fk_validation(
    ...
    This is analysis-only and does not affect training.
    """
    model.eval()
    fk_errors = []
    ...
    for x, y in tqdm(loader, desc="Validate-FK", leave=False):
        ...
        # Match evaluation protocol with A-4.3.x: compare predicted FK vs GT joints -> FK.
        theta_pred = model(x)
    ...
    summary_lines = [
        "A-4.4.1 Baseline (MSE-only; no FK supervision)",
    ...
    print(
        "FK Error Stats (validation — A-4.4.1 MSE-only): "
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="A-4.4.1 Baseline (MSE-only, no FK supervision)")
...
def main() -> None:
    ...
    for epoch in range(args.epochs):
        ...
        # Required training log: only joint-space MSE.
        print(f"[Epoch {epoch + 1:03d}] Train MSE: {train_mse:.4f}")

    # Validation FK analysis (no gradients, analysis-only).
    ...
    config = {
        ...
        "lr": args.lr,
        "seed": args.seed,
        "model": "IKNet_A4_4_1_MSE_only",
    }
    output_dir.mkdir(parents=True, exist_ok=True)
    with open(output_dir / "config.json", "w", encoding="utf-8") as f:
        json.dump(config, f, indent=2)

    # Optional checkpoint save (not required for the baseline but kept for parity).
    if args.save_path:
        save_path = Path(args.save_path)
        save_path.parent.mkdir(parents=True, exist_ok=True)
        torch.save(
            {
                "checkpoint": "A-4.4.1_MSE_only",
